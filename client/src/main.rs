...
use raylib::{
    color::Color,
    ffi::{
        BeginDrawing, ClearBackground, CloseWindow, ConfigFlags, DrawFPS, EndDrawing, InitWindow,
        SetConfigFlags, SetTargetFPS, WindowShouldClose,
    },
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    unsafe {
        ...
        let title = CString::new("Einkrad").unwrap();
        InitWindow(1024, 768, title.as_ptr());
        SetTargetFPS(60);
        ...
        for entry in data.read_dir()? {
            ...
            if entry.metadata()?.is_dir() {
                ...
                match Package::<ServiceMessage>::load(entry.path(), move |c| {
                    ...
                    let func =
                        c.create_function(|_lua, _: ()| Ok(LuaNode { inner: Node::new() }))?;
                    node.set("new", func)?;
        ...
        while !WindowShouldClose() {
            ...
            BeginDrawing();
            ClearBackground(Color::RAYWHITE.into());
